...
document.querySelectorAll('.accordion_item').forEach((item, index) => {
  item.querySelector('.accordion_toggle_text-number').textContent = String(index + 1).padStart(
    2,
    '0'
  )
})
...
function initMiniShowreelPlayer() {
  ...
  const safe = (t) =>
    t.closest('[data-mini-showreel-safearea]') || q('[data-mini-showreel-safearea]', t) || t
  ...
  function setStatus(status) {
    if (!n) return
    document
      .querySelectorAll(`[data-mini-showreel-lightbox="${n}"], [data-mini-showreel-player="${n}"]`)
...
const initFaqLoadMore = () => {
  ...
  const btn =
    wrapper.closest('.accordion_wrap')?.querySelector('.btn.is-load-more') ||
...
function initPreviewFollower() {
  ...
  wrappers.forEach((wrap) => {
    ...
    // Pre-create all visuals once and hide them — avoids DOM thrashing on every hover
    const visuals = Array.from(items).map((item) => {
      const source = item.querySelector('[data-follower-visual]')
      if (!source) return null
      const clone = source.cloneNode(true)
      gsap.set(clone, { yPercent: offset, autoAlpha: 0 })
    ...
    items.forEach((item, index) => {
      item.addEventListener('mouseenter', () => {
        ...
        if (prevIndex !== null && prevIndex !== index) {
          const outgoing = visuals[prevIndex]
          if (outgoing) {
        ...
        if (firstEntry) {
          ...
          gsap.fromTo(
            incoming,
            { yPercent: forward ? offset : -offset, autoAlpha: 0 },
    ...
    // If pointer leaves the collection, hide the active visual
    collection.addEventListener('mouseleave', () => {
      if (prevIndex !== null && visuals[prevIndex]) {
        const el = visuals[prevIndex]
        gsap.killTweensOf(el)
        gsap.to(el, { yPercent: -offset, autoAlpha: 0, duration, ease, overwrite: 'auto' })
      }
      firstEntry = true
      prevIndex = null
    })
...
function initMaskTextScrollReveal() {
  document.querySelectorAll('[data-split="heading"], [data-split="body"]').forEach((el) => {
...
function initVideoPlayback() {
  ...
  wrappers.forEach((wrapper, i) => {
    ...
    video.style.cssText =
      'position:absolute;inset:0;object-fit:cover;opacity:0;transition:opacity 0.3s ease;'
    ...
    if (wrapper.hasAttribute('data-video-on-hover')) {
      const trigger =
        wrapper.closest('[data-video-on-hover-trigger]') || wrapper.parentElement || wrapper
